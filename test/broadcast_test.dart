...
void main() {
  group('Broadcast Cosmos coin - ', () {
    test('Cosmos', () async {
      ...
      final request = await Broadcast.cosmos(
  ...
  group('Broadcast Ethereum coin - ', () {
    test('BNB Smart Chain', () async {
      ...
      final request = await Broadcast.ethereumRPC(
        signedTxEncoded: signedTxEncoded,
        apiEndpoint: 'https://bsc-dataseed.binance.org/',
      );
      expect(jsonDecode(request), isMap);
      expect(jsonDecode(request)["error"]["code"], -32000);
    });
    test('Ethereum', () async {
      const signedTxEncoded =
    ...
    test('Ethereum Classic', () async {
      ...
      final request = await Broadcast.ethereumBlockbook(
        signedTxEncoded: signedTxEncoded,
        apiEndpoint: 'https://etcblockexplorer.com/',
      );
      expect(jsonDecode(request), isMap);
      expect(jsonDecode(request)["error"],
  ...
  test('Broadcast solana', () async {
    ...
    final request = await Broadcast.solana(
        signedTxEncoded: signedTxEncoded,
        apiEndpoint: 'https://api.devnet.solana.com/');
    expect(jsonDecode(request)["error"]["code"], equals(-32002));
    expect(
      jsonDecode(request)["error"]["message"],
  ...
  group('Broadcast utxo coin - ', () {
    test('Bitcoin', () async {
      ...
      final request = await Broadcast.utxoCoinBlockbook(
          signedTxEncoded: signedTxEncoded,
          apiEndpoint: 'https://btc1.simplio.io/');
      expect(jsonDecode(request), isMap);
      expect(jsonDecode(request)['error'], contains('-22'));
    });
    ...
    test('Flux', () async {
      const signedTxEncoded =
          '0100000000010345e866343b494d89af0b75d15b56959e35280be401ee735920bba6a0c131436e';
      final request = await Broadcast.utxoCoinInsight(
          signedTxEncoded: signedTxEncoded,
          apiEndpoint: 'https://explorer.runonflux.io/');
      expect(request, 'TX decode failed. Code:-22');
    });
    test('Litecoin', () async {
      const signedTxEncoded =
          '0100000000010345e866343b494d89af0b75d15b56959e35280be401ee735920bba6a0c131436e';
